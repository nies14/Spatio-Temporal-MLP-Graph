...
def print_error(data_type,action_error_sum,show_protocol2=False,epoch=0,logger=None):


    if data_type =='h36m':
        mean_error =  print_error_action(action_error_sum, show_protocol2,epoch,logger)
...
def print_error_action(action_error_sum, show_protocol2=False,epoch=0,logger=None):
    ...
    if show_protocol2 :
        ...
        logger.append([epoch, mean_error_all['p1'].avg, mean_error_all['p2'].avg])
    else:
        ...
        logger.append([epoch, mean_error_all['p1'].avg])
...
def save_model(previous_wj_gcn_name, save_dir,epoch, save_out_type, data_threshold, model, model_name):
    if os.path.exists(previous_wj_gcn_name):
        os.remove(previous_wj_gcn_name)
